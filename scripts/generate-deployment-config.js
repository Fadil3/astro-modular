  ...
  for (const line of lines) {
    ...
    if (trimmed.includes(":") && (!inArray || !trimmed.startsWith("  "))) {
      ...
      if (value.startsWith("[")) {
        ...
        if (value !== "[") {
          // Single line array
          const arrayContent = value.substring(
            1,
            value.endsWith("]") ? value.length - 1 : value.length
          );
          if (arrayContent.trim()) {
            currentValue = arrayContent
              .split(",")
...
async function updateAstroConfig(redirects) {
  ...
  try {
    ...
    const newRedirectsSection = `redirects: ${JSON.stringify(
      redirectsObj,
      null,
      2
    ).replace(/"/g, "'")}`;
  ...
  if (currentPlatform !== "github-pages") {
    ...
    for (const file of githubPagesFiles) {
      try {
        ...
        log.info(
          `🧹 Removed ${path.basename(file)} (GitHub Pages not selected)`
        );
      } catch (error) {
...
`;
    ...
    log.info(
      `📝 Created public/_headers for GitHub Pages (requires paid plan)`
    );
  } catch (error) {
